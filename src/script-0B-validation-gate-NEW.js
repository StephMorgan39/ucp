...
async function main() {
    ...
    const pendingRecords = stagingQuery.records.filter(r =>
    ...
    for (const stagingRec of pendingRecords) {
            ...
            if (lastKnown !== undefined && incomingCost !== null) {
                if (Math.abs(incomingCost - lastKnown) > 0) {
                    const pct       = lastKnown !== 0
                        ...
                        : "N/A";
                    const direction = incomingCost > lastKnown ? "▲ increase" : "▼ decrease";
                    const detail    =
        ...
        if (pmId && inZWindow) {
            ...
            if (
                currentPMStatus === "Active" &&
                (isDiscontinued || isEOR)
            ) {
                const signal = isDiscontinued ? "Is_Discontinued = true" : "Is_End_Of_Range = true";
                holds.push({
                    rule: "R5",
        ...
        if (holds.length > 0) {
            ...
            stagingUpdates.push({ id: stagingRec.id, fields: {
            ...
            const errorTypeMap = { R1:"Price_Spike", R2:"Zero_Price", R4:"Description_Drift", R5:"Stock_Logic_Error" };

            for (const h of holds) {
                anomCreates.push({ fields: {
            ...
            adminNotes.push({ fields: {
                ...
                "fldPdoc6JPYHV9gpb": { name: holds.some(h => h.severity === "High") ? "High" : "Medium" },
            ...
            for (const info of infoLogs) {
                adminNotes.push({ fields: {
                    "fld4l6AJhVNRzIaY8":
                        `Script 0B ℹ️ SKU ${rawSku} passed. [${info.rule}] ${info.label}: ${info.detail}`,
                    "flda8oHUThBc1Kb7I": { name: "Missing_Data" },
    ...
    try {
        if (stagingUpdates.length > 0) {
            console.log(`Holding ${stagingUpdates.length} Staging records → pending_review...`);
