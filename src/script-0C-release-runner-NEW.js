...
async function main() {
    ...
    const staffQ = await staffTable.selectRecordsAsync({ fields: [STAFF_EMAIL_FID, STAFF_NAME_FID] });
    ...
    if (!whitelist.length) {
        output.markdown("## ❌ Cannot proceed — UtileStaff table has no email records.");
        output.markdown(
    ...
    const operatorEmail = await input.textAsync("Enter your email address to authorise release:");
    ...
    const anomQ = await anomTable.selectRecordsAsync({
        fields: [ANOM_APPROVED_FID, ANOM_APPROVED_BY_FID, ANOM_RESOLUTION_FID,
                 ANOM_SOURCE_RECORD_FID, ANOM_ERROR_TYPE_FID, ANOM_NOTES_FID]
    });
    ...
    output.markdown(`Approved anomalies awaiting release: **${approved.length}**`);
    if (!approved.length) {
        output.markdown("✅ Nothing to release. Check the Anomalies table — tick 'Approved' on records you want to release, then re-run.");
        return;
    }
    ...
    const confirm = await input.buttonsAsync(
        ...
        [{ label: "✅ Release", variant: "primary" }, { label: "❌ Cancel", variant: "danger" }]
    );
    if (confirm !== "✅ Release") {
    ...
    const approvedSkus = approved.map(r => {
    ...
    if (stagingUpdates.length > 0) {
        ...
        output.markdown(`✅ Successfully released **${stagingUpdates.length}** Staging rows back to PENDING.`);
    } else {
        output.markdown(`ℹ️ No matching Staging rows found in 'pending_review' state.`);
    }

    // ── Step 6: Log ─────────────────────────────────────────
    const logTable    = base.getTable(LOG_TABLE_ID);
    const supplierQ   = await base.getTable(SUPPLIER_TABLE_ID).selectRecordsAsync({
        fields: ["fld0Q6a6nO342Wfno"] // Supplier_Name_Pk — FIX: was loading no fields; records[0] is non-deterministic without a sort key
    });
    // Use first alphabetically sorted supplier as the log anchor (consistent across runs)
    const supplierRec = supplierQ.records.sort((a, b) =>
        a.getCellValueAsString("fld0Q6a6nO342Wfno").localeCompare(b.getCellValueAsString("fld0Q6a6nO342Wfno"))
    )[0];
    ...
    await logTable.createRecordsAsync([{
        fields: {
            ...
            [LOG_TYPE_FID]:     { name: "Missing_Data" }, // closest available type for release events
            ...(supplierRec ? { [LOG_SUPPLIER_FID]: [{ id: supplierRec.id }] } : {})
        }
    }]);
    ...
    output.markdown(`- Next step: Set released Staging rows back to PENDING, then run Script 1.`);
}
