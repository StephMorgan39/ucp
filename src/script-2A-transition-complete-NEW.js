...
async function main() {
  ...
  const matchedOption = validOptions.find(
    (o) => o.toLowerCase() === ARCHIVE_REASON_VALUE.toLowerCase(),
  );
  if (!matchedOption) {
    throw new Error(
  ...
  if (DRY_RUN)
    output.markdown("> ⚠️ **DRY RUN MODE** — no records will be written.");
  output.markdown("---");
  ...
  const allFlagged = pmQuery.records.filter((r) => {
    const route = r
      .getCellValueAsString("fldSZsiBxKNTHPFel")
      .trim()
      .toUpperCase();
  ...
  output.markdown(
    `Found **${allFlagged.length}** B-suffix records pending transition. Running pre-flight validation...`,
  );
  output.markdown("---");
  ...
  for (const rec of allFlagged) {
    ...
    const skuDisplay =
      rec.getCellValueAsString("fldmvzTtvWLaKetZZ") || skuMaster;
    ...
    if (upperSKU.endsWith("Z") || upperSKU.endsWith(".Z")) {
      ...
      rejectedRecords.push({
        ...
        skuDisplay,
        reason:
          "SKU ends in Z — this is a legacy record. Unflag it and flag the corresponding .B record instead.",
      });
      ...
      if (links.length === 0) {
        rejectedRecords.push({
          ...
          skuDisplay,
          reason:
            "No .B suffix and no Transition_Linked_SKU — nothing to do. Unflag this record.",
        });
      } else {
  ...
  if (rejectedRecords.length > 0) {
    ...
    output.markdown(
      "The following records are **flagged incorrectly** and will be **skipped**:",
    );
  ...
  if (validRecords.length === 0) {
    output.markdown(
      "## ❌ No valid records to process after pre-flight check.",
    );
  ...
  output.markdown(
    `## ✅ ${validRecords.length} valid record(s) will be processed.`,
  );

  // Preview valid records
  const previewRows = validRecords
    .map((r) => {
      const sku =
        r.getCellValueAsString("fldmvzTtvWLaKetZZ") ||
        r.getCellValueAsString("fldMfK3uyPnDbKONn");
      const links = r.getCellValue("fldvqYzX3ZVB1UsRi") || [];
      return `| ${sku} | ${links.length} linked .Z SKU(s) |`;
    })
    .join("\n");
  output.markdown(
    "| Active SKU (.B) | Transition Links |\n|---|---|\n" + previewRows,
  );
  ...
  if (linkedIds.size > 0) {
    ...
    output.markdown(
      "ℹ️ No linked .Z records found — will only perform suffix drops.",
    );
  }
  ...
  if (rejectedRecords.length > 0) {
    output.markdown(
      `> ⚠️ **${rejectedRecords.length} record(s) will be skipped** (see pre-flight warnings above).`,
    );
  }
  ...
  for (const activeRec of validRecords) {
    const skuMaster = activeRec
      .getCellValueAsString("fldMfK3uyPnDbKONn")
      .trim();
    const skuDisplay =
      activeRec.getCellValueAsString("fldmvzTtvWLaKetZZ") || skuMaster;
    ...
    try {
      ...
      for (const link of linkedLegacy) {
        ...
        const legSKU =
          legRec.getCellValueAsString("fldmvzTtvWLaKetZZ") ||
          legRec.getCellValueAsString("fldMfK3uyPnDbKONn");
        ...
        const dataDump = JSON.stringify(
          {
            ...
            upc_body_class: (
              legRec.getCellValue("fldyrCQE5L3lQktS3") || []
            ).map((l) => l.id),
            upc_body_finish: (
              legRec.getCellValue("fldU1VARr6kM7sihI") || []
            ).map((l) => l.id),
            upc_look_finish: (
              legRec.getCellValue("fld5wEWzTjID4sIAE") || []
            ).map((l) => l.id),
            upc_design_style: (
              legRec.getCellValue("fldMjjmaeriqqcxiS") || []
            ).map((l) => l.id),
            upc_colour_master: (
              legRec.getCellValue("fldDBJifgrvsMqR9g") || []
            ).map((l) => l.id),
          },
          null,
          2,
  ...
  try {
    if (legacyCreates.length > 0) {
      output.markdown(
        `Creating ${legacyCreates.length} LegacyCodes records...`,
      );
    ...
    if (adminLogCreates.length > 0) {
      ...
      output.markdown(
        `✅ ${adminLogCreates.length} AdminLog warnings written.`,
      );
    }
  } catch (e) {
