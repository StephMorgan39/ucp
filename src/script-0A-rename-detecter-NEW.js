...
async function main() {
  ...
  const pendingRows = stagingQ.records.filter((r) => {
    ...
    const type = r
      .getCellValueAsString(STG_IMPORT_TYPE_FID)
      .trim()
      .toUpperCase();
  ...
  const dotARows = pendingRows.filter((r) =>
    r
      .getCellValueAsString(STG_SUPPLIER_SKU_FID)
      .trim()
      .toUpperCase()
      .endsWith(".A"),
  );
  ...
  for (const dotARow of dotARows) {
    ...
    console.log(
      `- \`${rawSku}\` → base \`${baseSku}\` ${hasPair ? "✅ pair found in same batch" : "⚠️ no base code in this batch (will be handled on next run)"}`,
    );
    ...
    pmQ.records
      .filter((r) =>
        ["ACTIVE", "NEW PRODUCTS", "SKU_Transition"].includes(
          r.getCellValueAsString(PM_STATUS_FID).trim(),
        ),
      )
      .map((r) =>
        r.getCellValueAsString(PM_SKU_MASTER_FID).trim().toUpperCase(),
      ),
  );
