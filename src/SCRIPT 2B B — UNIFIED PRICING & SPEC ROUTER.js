// =============================================================
// SCRIPT 2 — TRANSITION COMPLETION (SKU Suffix Drop) v1.2
...
//   Processes ProductMaster records where Manual Flag For Migration = true.
...
async function main() {
    ...
    if (!matchedOption) {
        throw new Error(
            ...
            `Update the ARCHIVE_REASON_VALUE constant at the top of the script to one of these exact values.`
        );
    }
    ...
    const pmQuery = await pmTable.selectRecordsAsync({
        fields: [
            // ⚠️ FIX REQUIRED: fldpoE4p3L8V6P4oi (Manual Flag For Migration) is NOT in the
            // current schema export. You must either:
            //   a) Create a checkbox field on ProductMaster called "Manual Flag For Migration"
            //      and replace fldpoE4p3L8V6P4oi with its real field ID, OR
            //   b) Use an existing checkbox field that serves this purpose.
            // Script will fail at runtime until this is resolved.
            "fldpoE4p3L8V6P4oi", // ← REPLACE with real field ID after creating the field
    ...
    const allFlagged = pmQuery.records.filter(r =>
        r.getCellValue("fldpoE4p3L8V6P4oi") === true
    );

    output.markdown(`Found **${allFlagged.length}** flagged records. Running pre-flight validation...`);
    output.markdown("---");
    ...
    if (rejectedRecords.length > 0) {
        ...
        output.markdown("---");
        output.markdown("**Action required:** Go to ProductMaster and uncheck `Manual Flag For Migration` on the records above, then flag the correct `.B` counterparts.");
    }
    ...
    if (linkedIds.size > 0) {
        const legacyQuery = await pmTable.selectRecordsAsync({
            fields: [
                ...
                "fldpoE4p3L8V6P4oi", // Manual Flag For Migration
    ...
    output.markdown(
        ...
        `- Clear Manual Flag For Migration\n` +
    ...
    for (const activeRec of validRecords) {
        ...
        try {
            ...
            pmUpdates.push({ id: activeRec.id, fields: {
                ...
                "fldpoE4p3L8V6P4oi": false,     // Clear flag
            }});
            countDropped++;
