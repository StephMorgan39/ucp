...
const STD_CATEGORY = "fld2bslzLVAsSQIT8";
...
function parseNumeric(val) {
  ...
  // Detect European format: comma as decimal, period as thousands
  // e.g. "1.500,50" → 1500.50 vs "1,500.50" → 1500.50
  if (/^\d{1,3}(\.\d{3})+(,\d+)?$/.test(s)) {
    // European: "1.500,50" → strip periods → swap comma to period
    s = s.replace(/\./g, "").replace(",", ".");
  } else {
    // Standard or ambiguous: strip commas as thousands, replace comma-as-decimal
...
function translateValue(stdEngine, canonicalId, rawVal, fieldName) {
  ...
  if (match) {
    ...
    return {
      value: sanitized,
      warning: `Standardization required for ${fieldName}: "${sanitized}"`,
    };
  }
}
...
async function main() {
  ...
  for (const r of stdQuery.records) {
    ...
    const category = r.getCellValueAsString(STD_CATEGORY).trim();

    if (!category) continue; // skip rows with no Category — they can't be keyed
    const key = `${category.toUpperCase()}|${sanitizeRaw(inputVal).toUpperCase()}`;
  ...
  const pbQuery = await pbTable.selectRecordsAsync({
    fields: [PB_PRODUCT_SKU, PB_PRICE_TYPE, PB_COST, PB_PRICE_STATUS],
  });
  const pbIndex = {};
  for (const rec of pbQuery.records) {
    ...
    if (pmLinks.length > 0 && pt)
      pbIndex[`${pmLinks[0].id}|${pt.toLowerCase()}`] = rec; // FIX: was pmLinks.id
  }
  ...
  for (const stagingRec of prRecords) {
    ...
    if (rawType.toLowerCase().includes("sample supplier"))
      priceType = "Sample supplier-charged";
    ...
    if (supplierLinks.length > 0)
      pbFields[PB_SUPPLIER] = [{ id: supplierLinks[0].id }]; // FIX: was supplierLinks.id
    ...
    const parsedThick = parseNumeric(
      stagingRec.getCellValueAsString(S_THICKNESS),
    );
    if (parsedThick !== null) {
    ...
    const parsedFaces = parseNumeric(
      stagingRec.getCellValueAsString(S_NO_FACES),
    );
  ...
  if (!DRY_RUN) {
    ...
    if (pbCreates.length > 0)
      for (const b of chunk(pbCreates, 50)) await pbTable.createRecordsAsync(b);
    if (pbUpdates.length > 0)
      for (const b of chunk(pbUpdates, 50)) await pbTable.updateRecordsAsync(b);
    if (pmUpdates.length > 0)
      for (const b of chunk(pmUpdates, 50)) await pmTable.updateRecordsAsync(b);
